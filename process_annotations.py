...
def save_training_data(training_data, output_file):
    ...
    print(f"[OK] Saved {len(training_data)} annotations to {output_file}")
...
def save_csv(training_data, output_file):
    ...
    print(f"[OK] Saved CSV summary to {output_file}")
...
def create_class_mapping(training_data):
    ...
    print(f"\n[OK] Created class mapping with {len(class_mapping)} classes")
...
def main():
    ...
    if not Path(args.export_file).exists():
        print(f"[X] Error: File '{args.export_file}' not found")
    ...
    print(f"[OK] Loaded {len(annotations)} items")
    
    print("\nProcessing annotations...")
    training_data = process_annotations(annotations)
    
    if not training_data:
        print("[X] No annotated videos found in export file")
        print("Make sure you have submitted annotations in Label Studio")
        return
    
    print(f"[OK] Processed {len(training_data)} annotated videos")
